...
--   * No ContextPtr:SetUpdate and no GameDataDirty scanner.
...
print("LEK Fair Trades v1.0: loading")
...
-- Slightly looser proactive cadence than stock diplomacy, while native CvDealAI
-- still determines the actual price. Hostile/deceptive/war never get synthetic offers.
...
local function SpareLuxuries(playerID, otherID)
    ...
    for res in GameInfo.Resources() do
        ...
        if IsLuxury(rid) then
            ...
            -- Never seed a last copy, and do not offer a luxury the recipient already has.
...
local function RunNativeSeed(aiID, humanID, seedFromHuman, resourceID)
    ...
    local ok, err = pcall(function()
        -- Mirror the stock Trade button initialization before asking CvDealAI
        -- to construct the counter-price, but do it headlessly and close at once.
...
-- Native UI bridge without modifying LeaderHeadRoot.lua. We wait for the normal
-- leader root generated by DoBeginDiploWithHuman(), initialize the stock trade
-- state, rebuild the already-validated scratch deal, then emit the game's normal
-- AI_MAKES_OFFER state that EUI TradeLogic already handles.
...
local function TryAI(aiID, humanID, turn)
    ...
    -- Alternate which direction gets first chance so the AI sometimes buys and
    -- sometimes sells. Each path asks the native AI to construct the other side.
...
StateSet("PerformanceModel", "ONE_TURN_SCAN_MAX_8_NATIVE_HELPERS_NO_UPDATE_LOOP")
...
print("LEK Fair Trades v1.0: ready - native seed engine, max 8 helper calls/turn")
