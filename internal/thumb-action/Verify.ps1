...
try {
    ...
    $checks=[ordered]@{
        ...
        'settler settles immediately when legal, else moves to nearest recommendation'=($t.Contains('owner:CanFound') -and $t.Contains('"MISSION_FOUND"') -and $t.Contains('Map.PlotDistance(x,y,p:GetX(),p:GetY())'))
        'stacked-unit blocker moves off the stack instead of fortifying'=($t.Contains('isStackedBlocker') -and $t.Contains('LEKSpaceMoveSelectedUnit(p,"UNSTACK_MOVE")'))
